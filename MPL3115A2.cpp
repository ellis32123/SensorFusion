...
float MPL3115A2::readRawBits(){
    toggleOneShot();
    byte* dataBuffer = read(STATUS,NUM_OF_OUTPUT_BYTES);
    float tempcsb = (dataBuffer[2]>>4)/16.0;

	return (float)( (dataBuffer[0] << 8) | dataBuffer[1]) + tempcsb;
    //return  (float)((dataBuffer[0] << 16) | (dataBuffer[1] << 8) | (dataBuffer[0] << 4));
}
...
float MPL3115A2::readAltitude()
{
    ...
    while( (IIC_Read(STATUS) & (1<<1)) == 0){
		...
		//delay(1);
		if(current_micros - micros() > MAX_WAIT_MICROS)
            return -999;
        else
            delay(1);
	}
...
void MPL3115A2::write(int address, int data){
    Wire.beginTransmission(MPL3115A2_ADDRESS);
    Wire.write(address);
    Wire.write(data);
...
uint8_t* MPL3115A2::read(int address, int number_of_bytes){
    ...
    Wire.beginTransmission(MPL3115A2_ADDRESS);
    Wire.requestFrom(MPL3115A2_ADDRESS, number_of_bytes);
...
void MPL3115A2::strobeOneShot(){
  uint8_t* tempSetting_pointer = read(CTRL_REG1,1); //Read current settings
  int tempSetting = *tempSetting_pointer ;
  tempSetting &= ~(1<<1); //Clear OST bit
  write(CTRL_REG1, tempSetting);

  tempSetting_pointer = read(CTRL_REG1,1); //Read current settings to be safe
  tempSetting = *tempSetting_pointer ;
  tempSetting |= (1<<1); //Set OST bit
  write(CTRL_REG1, tempSetting);
}

void MPL3115A2::setOversample(byte sampleRate){
  if(sampleRate > 7) sampleRate = 7; //OS cannot be larger than 0b.0111
  sampleRate <<= 3; //Align it for the CTRL_REG1 register
